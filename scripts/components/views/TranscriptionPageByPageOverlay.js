...
export default class TranscriptionPageByPageOverlay extends React.Component {
	constructor(props) {
		super(props);
			...
			window.eventBus.addEventListener('overlay.transcribePageByPage', function(event) {
				if(event.target.random){
					...
					this.setState({
						...
						var currentImage = images[this.state.imageIndex];
						var currentImageSource = currentImage.source;
						if (currentImage.transcriptionstatus === "readytotranscribe") {
							this.transcribeStart(event.target.id, currentImageSource);
						}
					}.bind(this));
				}
			}.bind(this));
